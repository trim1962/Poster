...
%	\begin{center}
		...
		\begin{tikzpicture}[scale=1, %node distance = 2.5cm,
			...
			\node [input, below of=init] (passo1) {Leggo le coordinate del punto $A(x_1,y_1)$};
	\node [input, below of=passo1, node distance=2cm] (passo2) {Leggo le coordinate del punto $B(x_2,y_2)$};		
		\node [block, below of=passo2, node distance=2cm] (passo3) {Sommo $x_1+x_2$};
		\node [block, below of=passo3, node distance=2cm] (passo4) {Sommo $y_1+y_2$};	
		\node [print, below of=passo4, node distance=2cm] (passo5) {Scrivo $M\left(\dfrac{x_1+x_2}{2},\dfrac{y_1+y_2}{2}\right)$};
		
	\node [cloud, below of=passo5] (stop) {Fine};
