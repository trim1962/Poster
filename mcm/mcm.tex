...
radius=5pt,
		...
		\begin{tikzpicture}[scale=1, %node distance = 2.5cm,
			...
			\node[block,below of=sceltasi1,node distance=2cm] (passo2a) {Prendo i fattori comuni e non comuni con il maggiore esponente.};
			\node[print,below of=passo2a,node distance=2cm] (passo3) {$\mcm$ è il prodotto dei fattori trovati};
			\node[cloud,below of=passo3](fine){Fine};
			...
			\path[line](sceltasi1)--(passo2a);
			\path[line](passo2a)--(passo3);
			\path[line](passo3)--(fine);
	\end{tikzpicture}
\end{document}