...
radius=5pt,
		...
		\node (zero) {Forma normale $ax^2+bx+c=0$};
			...
			\node [decision, below of=init,node distance=3cm] (decisione1) {L'equazione è in forma normale?};
			\node[block, right of=decisione1, node distance=4cm] (dec1No) {Sommo i termini simili};
			\node[block, below of=decisione1, node distance=4cm] (dec1Si) {Cerco il termine di secondo grado};
