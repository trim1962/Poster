...
%	\begin{center}
		...
		\begin{tikzpicture}[scale=1, %node distance = 2.5cm,
			...
			\node [block, below of=decisione1,node distance=3cm] (sceltasi1) {La trascrivo uguale a sinistra (prima) dell'uguale};
			\node [block, right of=decisione1,node distance=6cm] (sceltano1) {La trascrivo cambiata di segno a sinistra  prima dell'uguale};
			\node[connessione,below of=sceltasi1,node distance=1cm] (nodo1) {};
