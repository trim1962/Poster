...
%	\begin{center}
		...
		\begin{tikzpicture}[scale=1, %node distance = 2.5cm,
			...
			\node [decision, below of=passo2,node distance=2cm] (decisione1) {L'incognita è a sinistra (prima) dell'uguale?};
		...
		\node [input, below of=nodo3,node distance=2cm] (passo3) {Leggo l'espressione da sinistra verso destra cercando i numeri};
...
\node[connessione,below of=sceltasi3,node distance=2cm] (nodo4) {};
\node [decision, below of=nodo4] (decisione4) {Arrivato a destra?};
\node[block,below of=decisione4,node distance=2.5cm] (passo4) {Sommo le incognite};
\node[block,below of=passo4,node distance=1.5cm] (passo5) {Sommo i numeri};
