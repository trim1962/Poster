...
%	\begin{center}
		...
		\begin{tikzpicture}[scale=1, %node distance = 2.5cm,
			...
			\node[block,below of=decisione1,node distance=2cm
			] (sceltasi1) {Allineo le scomposizioni ottenute};
			\node[decision,below of=sceltasi1,node distance=3cm] (decisione2) {Vi sono fattori comuni?};
			...
			\node[block,below of=sceltasi2,node distance=2cm] (passo3) {$\MCD$ è il prodotto dei fattori trovati};
		\end{tikzpicture}
%	\end{center}
\end{document}