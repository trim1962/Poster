...
%	\begin{center}
		...
		\begin{tikzpicture}[scale=1, %node distance = 2.5cm,
			...
			\node[connessione, below of=passo4] (con1) {};
			\node [decision, below of=con1] (decisione1) {Esiste un numero che divide numeratore uno e denominatore due?};
		\node [block, below of=decisione1,node distance=3.5cm] (passo5) {Divido numeratore uno e il denominatore due per questo numero};
		\node[connessione,below of=passo5] (nodo2) {};
				\node [decision, below of=nodo2, node distance=3cm] (decisione2) {Esiste un numero che divide numeratore due e denominatore uno?};
			...
			\path [line] (passo1) -- (passo2);
     		...
     		\path [line] (passo4) --  (con1);
     		\path [line] (con1) --  (decisione1);
			\path [line] (decisione1.east) --++(1,0) node [near start] {No}|-(nodo2);
...
\path [line] (decisione2) --  node [near start] {Si}(passo6);
			\path [line] (passo5.west)-- ++(-1,0)|-(con1);
			\path [line] (passo6.west)-- ++(-1,0)|-(nodo2);
		\end{tikzpicture}
%	\end{center}
\end{document}